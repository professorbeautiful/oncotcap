...
public class MersenneTwister implements Serializable
{
	...
	//new setSeed method
	//use both halfs (int's) of the long instead of
	//just one to seed the RNG.  
	public final void setSeed(final long seed)
	{
		int i;
		savedSeed = new Long(seed);

		int _seed = (int) (seed & 0xffffffff);
		int _seed2 = (int) (seed >> 32);
		...
		for (i=0;i<N/2;i++)
		{
			mt[i] = _seed2 & 0xffff0000;
			_seed2 = 69069 * _seed2 + 1;
			mt[i] |= (_seed2 & 0xffff0000) >>> 16;
		}
		for (i=N/2;i<N;i++)
		{
     ...
     *
     */
	// this is the original setSeed method from MT,
	// replaced by the one above.  wes 4/10/2002
	public final void setSeedi(final long seed)
    {
		savedSeed = new Long(seed);
    ...
    { 
		...
		/*
		  ...
		  r.setSeedOld(4357)
		  Logger.log("Output of MersenneTwister, old style");
		  for (j=0;j<1000;j++)
		  {
		...
		/*
		  r = new MersenneTwister(4357);
		  Logger.log("Output of MersenneTwister with new (1999/10/28) seeding mechanism");
		  for (j=0;j<1000;j++)
		  {
		  // first, convert the int from signed to "unsigned"
		  long l = (long)r.nextInt();
		...
		*/
		...
		/*
		  ...
		  xx += r.nextInt();
		  Logger.log("Mersenne Twister: " + (System.currentTimeMillis()-ms + "          Ignore this: " + xx));
		  ...
		  for (j = 0; j < 10000000; j++)
		  xx += rr.nextInt();
		  Logger.log("java.util.Random: " + (System.currentTimeMillis()-ms + "          Ignore this: " + xx));
		*/
	...
	*/
	/*	int n;
		MersenneTwister rng = new MersenneTwister(9998);
		for (n=1; n<=10; n++)
			Logger.log(rng.nextDouble());
		rng.setSeed(9998);
		Logger.log("########################");
		for (n=1; n<=10; n++)
			Logger.log(rng.nextDouble());	   
	*/
		...
		mt.setSeed(0x0ccccccc0aaaaaaaL);
		for(n = 1; n<=1000000; n++)
		{
			v = mt.nextDouble();
			if( (n%10000) == 0)
				Logger.log(new Double(n));
		}
